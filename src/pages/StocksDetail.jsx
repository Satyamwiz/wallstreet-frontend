...
const StocksDetail = () => {
  ...
  // Initialize stock state, or set later if not passed via state
  const [stock, setStock] = useState(passedState?.stock || null);
  const [availableShares, setAvailableShares] = useState(0);
  const [isMarketOpen, setIsMarketOpen] = useState(true);
  const [activeTab, setActiveTab] = useState("overview");
  ...
  // State for the continuously updating price from the socket
  const [currentPrice, setCurrentPrice] = useState(stock ? stock.price : 0);
  // State for the bookmark toggle
  const [isBookmarked, setIsBookmarked] = useState(false);

  // Fetch stock details if not provided via state
  useEffect(() => {
    if (!passedState || !passedState.stock) {
      stockService
        .getStockDetail(name)
      ...
      setStock(passedState.stock);
      setCurrentPrice(passedState.stock.price);
    }
    // Set available shares (mock value)
    setAvailableShares(100);
